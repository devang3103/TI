 ...
 *
 * This function takes the column pattern definition and writes the described
 ...
 * @return  â‰¤0  Error code as #DLPSPEC_ERR_CODE
 */
{
	...
	/*Return an error if there are no patterns provided and/or the frame buffer doesn't exsist*/
	if ((patDefCol == NULL) || (pFB == NULL))
		return (ERR_DLPSPEC_NULL_POINTER);
	
	/*copy the list of patterns to the frame buffer*/
	...
	curBuffer = startPattern/patterns_per_image;				/*=0*/
	frameBuffer.frameBuffer += ((frameBufferSz/4)*curBuffer);	/*=0*/
	curPattern = startPattern - curBuffer*patterns_per_image;	/*=startPattern*/
 ...
 */
{
	...
	/* return an error if lacking a configuration, coefficients, or pattern
	defintion */
	...
	/* computes the corresponding DMD column number given the starting wavelength */
	ret_val = dlpspec_util_nmToColumn(pScanConfig->wavelength_start_nm, 
			pCoeffs->PixelToWavelengthCoeffs, &start_px);
    if (ret_val < 0) /* return an error if the result is negative */
    {
        return (ERR_DLPSPEC_INVALID_INPUT);
    }
    
	/* computes the corresponding DMD column number given the ending wavelength */
	...
	/*if there is more than one pattern (shouldn't be in our case), find the distance
	between pattern centres */
 ...
 */
{
	...
	/* return an error if missing pResults or puScanData*/
    DLPSPEC_ERR_CODE ret_val = (DLPSPEC_PASS);
    ...
    */
	...
	dlpspec_copy_scanData_hdr_to_scanResults(puScanData, pResults); /* copy over all scan header info */
	
    dlpspec_subtract_remove_dc_level(pScanData, pResults); /* subtract black pattern from all other patterns */
